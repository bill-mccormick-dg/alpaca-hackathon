...
| `journal_viewer.py [--port 8300]` | The journal streamed to a browser over server-sent events — live follow, per-account filters, replay by date. LAN-bound and read-only; published at **[bot.wpmccormick.pw](https://bot.wpmccormick.pw)** through a Cloudflare tunnel. **Access is on request** — Cloudflare Access gates it on an explicit allow-list of email addresses signed in through Google, so the link shows a login wall until an address is added. Judges and reviewers: ask and it is added, and removed the same way. The demo video shows the viewer in use if you would rather not wait |
