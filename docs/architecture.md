...
| `bot/config.py` + `bot/overrides.py` | `config.yaml` + runtime overrides (allowlisted, expire 16:00 ET) | `config_provenance()` is what the journal records each cycle; `resolve_review_model()` picks a reviewer that did not trade the day, recomputed each call so a dashboard model swap cannot silently make an account its own reviewer. Since 2026-09-08 all three accounts share one model and no independent candidate exists, so it returns `None` and `eod_review.py` falls back to the trading model - a disclosed self-review, see docs/strategy.md |
