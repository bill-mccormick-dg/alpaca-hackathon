...
| `decision` | the model answered — raw output, model, token usage, latency, finish reason, reasoning head, tool calls, the size of the learning and positions blocks it was shown (`learning_chars`, `positions_block_chars`), and `citations`: how many prior figures its reasons quoted and which of them appear nowhere in the prior it was given (#172; `scripts/audit_citations.py` rebuilds it for older days) |
